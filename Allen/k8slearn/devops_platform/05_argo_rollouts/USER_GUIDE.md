...
**方式二：通过 patch 触发**
```bash
ssh root@182.42.82.135 'kubectl patch rollout web-service -n service-test \
  --type=merge \
  -p "{\"spec\":{\"template\":{\"metadata\":{\"annotations\":{\"release\":\"$(date +%s)\"}}}}}"'
```
...
```bash
# 推进一步
ssh root@182.42.82.135 'kubectl argo rollouts promote web-service -n service-test'

# 直接全量发布（跳过所有暂停）
ssh root@182.42.82.135 'kubectl argo rollouts promote web-service -n service-test --full'
```
...
```bash
# 中止发布，自动回滚到稳定版
ssh root@182.42.82.135 'kubectl argo rollouts abort web-service -n service-test'
```
...
```bash
# 先重置状态
ssh root@182.42.82.135 'kubectl argo rollouts retry rollout web-service -n service-test'
```
...
```bash
ssh root@182.42.82.135 'kubectl argo rollouts undo web-service -n service-test'
```
...
# 发布新版本
ssh root@182.42.82.135 'kubectl argo rollouts set image web-service web=182.42.82.135:30002/service-test/web-service:v2 -n service-test'
...
# === user-service ===
# 查看状态
ssh root@182.42.82.135 'kubectl argo rollouts get rollout user-service -n service-test'

# 发布新版本
ssh root@182.42.82.135 'kubectl argo rollouts set image user-service user=182.42.82.135:30002/service-test/user-service:v2 -n service-test'
```
...
**A**: 集成 Prometheus 分析，当错误率超过阈值时自动回滚（高级功能，需要额外配置）。
