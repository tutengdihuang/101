...
> 🎯 **一句话精华**：排查问题 = 看日志 + 查状态 + 测连通——三板斧下去，问题无处遁形！
...
**金句**：
```
"Target DOWN 三步走：查 Pod、查 Service、测连通"
```

---

### 2.2 ServiceMonitor 不生效
...
**常见原因**：
- 缺少 `release: prometheus` 标签（90% 的问题）
...
**金句**：
```
"ServiceMonitor 不生效？先查 release: prometheus 标签！"
```
...
```bash
# 1. 检查指标是否存在
curl "http://<prometheus-ip>:<port>/api/v1/label/__name__/values" | grep <metric_name>

# 2. 检查指标的标签
curl "http://<prometheus-ip>:<port>/api/v1/series?match[]={__name__=\"<metric_name>\"}"
...
```bash
# 1. 检查时间序列数量
curl http://<prometheus-ip>:<port>/api/v1/status/tsdb | python3 -m json.tool

# 2. 检查抓取目标数量
curl http://<prometheus-ip>:<port>/api/v1/targets | python3 -c "
import sys, json
data = json.load(sys.stdin)
print(f\"Active targets: {len(data['data']['activeTargets'])}\")"

# 3. 检查高基数指标（罪魁祸首！）
curl http://<prometheus-ip>:<port>/api/v1/status/tsdb | python3 -c "
import sys, json
data = json.load(sys.stdin)
...
curl http://<prometheus-ip>:<port>/api/v1/rules | grep <rule_name>
...
curl http://<alertmanager-ip>:<port>/api/v2/alerts | python3 -m json.tool

# 2. 检查是否被静默
curl http://<alertmanager-ip>:<port>/api/v2/silences | python3 -c "
import sys, json
data = json.load(sys.stdin)
for s in data:
...
### 5.2 检查 Prometheus 状态

```bash
# 检查配置
curl http://<prometheus-ip>:<port>/api/v1/status/config

# 检查运行时信息
curl http://<prometheus-ip>:<port>/api/v1/status/runtimeinfo

# 检查 TSDB 状态
curl http://<prometheus-ip>:<port>/api/v1/status/tsdb

# 检查所有 Target
curl http://<prometheus-ip>:<port>/api/v1/targets

# 检查所有规则
curl http://<prometheus-ip>:<port>/api/v1/rules

# 检查所有告警
curl http://<prometheus-ip>:<port>/api/v1/alerts
```

### 5.3 检查 AlertManager 状态

```bash
# 检查所有告警
curl http://<alertmanager-ip>:<port>/api/v2/alerts

# 检查所有静默
curl http://<alertmanager-ip>:<port>/api/v2/silences

# 检查状态
curl http://<alertmanager-ip>:<port>/api/v2/status
```
...
"90% 的 ServiceMonitor 问题都是标签问题"

"重启解决 80% 的问题，重装解决 99% 的问题"

"先看 describe，再看 logs，最后看配置"
```
...
- [ ] 会查看 Pod 状态和日志
- [ ] 会检查 ServiceMonitor 配置
- [ ] 会测试指标端点连通性
- [ ] 会查看 Prometheus Targets
- [ ] 会检查 AlertManager 告警和静默
- [ ] 会重启监控组件
...
> 📝 **系列导航**：
> - 上一篇：[告警静默操作](silence-alerts.md)
> - 返回：[监控系统概述](../01-monitoring-overview.md)
