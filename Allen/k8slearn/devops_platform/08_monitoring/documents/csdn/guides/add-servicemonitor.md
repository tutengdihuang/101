# 添加监控目标：让 Prometheus 发现你的应用

> 🎯 **一句话精华**：ServiceMonitor = 告诉 Prometheus "去哪里抓指标"——就像给快递员一个收件地址！

## 一、秒懂定位（30秒版）

**这个知识解决什么问题**：
```
你的应用已经暴露了 /metrics 端点
但 Prometheus 不知道去哪里抓取
ServiceMonitor 就是那个"地址簿"
```

**适合谁学**：需要让 Prometheus 监控自己应用的开发/运维人员
**不适合谁**：只使用内置监控的用户
...
# 等待 1-2 分钟后，检查 Prometheus Targets
...
如果应用没有 Service（比如批处理任务），可以使用 PodMonitor：
...
```
ServiceMonitor：通过 Service 发现 Pod（推荐）
PodMonitor：直接发现 Pod（适合没有 Service 的场景）
```
...
```
"ServiceMonitor 三要素：release 标签、namespace 选择、Service 匹配"

"Target 不出现？先查标签，再查 Service，最后查日志"

"没有 Service 用 PodMonitor，有 Service 用 ServiceMonitor"
```
...
> - 下一篇：[创建告警规则](create-alert-rule.md)
